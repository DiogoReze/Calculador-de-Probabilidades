...
# Adicione esta função no início do seu código
def ativar_desativar_alarme(janela):
    global alarme_ativo
    while alarme_ativo:
        janela.configure(bg='red')  # Altera a cor para vermelho
        time.sleep(5)
        janela.configure(bg='white')  # Restaura a cor original
        time.sleep(1)
...
class JanelaResultado(tk.Tk):
    def __init__(self):
        ...
        # Adicionando estilos para negrito e itálico
        negrito_italico = ('Arial', 20, 'bold italic')

        self.title("Probabilidades CandleStick")
        self.geometry("450x200")

        # Adicionando borda à janela
        self.configure(bg='white', highlightthickness=15, highlightbackground='black')

        # Adicionando widget Label para exibir a imagem
        self.label_imagem = tk.Label(self)
        self.label_imagem.pack()

        self.label_acuracia = tk.Label(self, text="Acurácia: ", anchor="w", font=negrito_italico)
        self.label_acuracia.pack()

        self.label_previBaixa = tk.Label(self, text="Queda: ", anchor="w", font=negrito_italico)
        self.label_previBaixa.pack()

        self.label_previDoji = tk.Label(self, text="Doji: ", anchor="w", font=negrito_italico)
        self.label_previDoji.pack()

        self.label_previAlta = tk.Label(self, text="Alta: ", anchor="w", font=negrito_italico)
        self.label_previAlta.pack()
        
        self.label_tempo_atual = tk.Label(self, text="Tempo Atual: ", anchor="w", font=negrito_italico)
        self.label_tempo_atual.pack()

    def atualizar_resultados(self, acuracia, previBaixa, previDoji, previAlta):
        # Atualizando os textos
        negrito_italico = ('Arial', 20, 'bold italic')
        self.label_acuracia.config(text=f"Acurácia: {acuracia:.2f}%", anchor="w", font=negrito_italico)
        self.label_previBaixa.config(text=f"Queda: {previBaixa:.2f}%", anchor="w", font=negrito_italico)
        self.label_previDoji.config(text=f"Doji: {previDoji:.2f}%", anchor="w", font=negrito_italico)
        self.label_previAlta.config(text=f"Alta: {previAlta:.2f}%", anchor="w", font=negrito_italico)

        tempo_atual = time.strftime('%H:%M:%S')
        self.label_tempo_atual.config(text=f"Horario: {tempo_atual}", anchor="w", font=negrito_italico)
    ...
    def desativar_alarme(self):
        global alarme_ativo
        alarme_ativo = False
        self.configure(bg='white')  # Restaura a cor original
